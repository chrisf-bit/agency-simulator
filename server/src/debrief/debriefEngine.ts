...
import { TeamState, GameState, ClientOpportunity } from '../types';
import {
  DebriefQuestion,
  TriggerResult,
  TeamAnalysis,
...
import { detectTriggers, GameContext } from './triggerDetection';
...
export function generateFacilitatorPrompts(gameState: GameState): FacilitatorPrompts {
...
// =============================================================================
// QUARTERLY PROMPTS (after each quarter)
// =============================================================================
...
export function generateQuarterlyPrompts(
  gameState: GameState,
  ...
  const perTeam = teams.map(team => {
    ...
    const recentTriggers = firedTriggers.filter(t =>
      t.quarters?.includes(currentQuarter) || t.quarters?.length === 0
    );
...
function getQuestionsForQuarter(currentQuarter: number, isEndOfGame: boolean): DebriefQuestion[] {
...
function buildGameContext(gameState: GameState): GameContext {
  const teams = Array.from(gameState.teams.values());

  // Count opportunities per quarter
  const opportunitiesPerQuarter = new Map<number, number>();
...
function generateGameNarrative(
  ...
  // Burnout events
  const burnoutTeams = analyses.filter(a =>
    a.triggers.some(t => t.type === 'burnout_spiral' || (t.type === 'high_burnout' && t.intensity > 0.7))
  );
  ...
  // Financial pressure
  const cashCrunchTeams = analyses.filter(a =>
    a.triggers.some(t => t.type === 'near_bankruptcy')
  );
  ...
  // Strategic contrast
  const aggressive = analyses.filter(a =>
    a.triggers.some(t => t.type === 'aggressive_pitching')
  );
  const selective = analyses.filter(a =>
    a.triggers.some(t => t.type === 'selective_pitching')
  );
